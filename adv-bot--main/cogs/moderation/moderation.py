...
class Moderation(commands.Cog):
  ...
  async def give(self, ctx, member: discord.Member, *, role: discord.Role):
    ...
    if role >= ctx.guild.me.top_role:
        ...
        error.set_author(name="Error", icon_url=ctx.bot.user.display_avatar.url)
    ...
    if ctx.author != ctx.guild.owner and ctx.author.top_role <= member.top_role:
        ...
        error.set_author(name="Access Denied", icon_url=ctx.bot.user.display_avatar.url)
        error.set_footer(text=f"Requested by {ctx.author}",
                        icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
        return await ctx.send(embed=error)
    ...
    try:
        if role not in member.roles:
            ...
            success.set_author(name="Role Added", icon_url=ctx.bot.user.display_avatar.url)
            ...
            success.set_author(name="Role Removed", icon_url=ctx.bot.user.display_avatar.url)
            success.set_footer(text=f"Requested by {ctx.author}",
                        icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
  ...
  async def clone(self, ctx: commands.Context, channel: discord.TextChannel):
    ...
    try:
        ...
        success.set_author(name="Success", icon_url=ctx.bot.user.display_avatar.url)
        success.set_footer(text=f"Requested by {ctx.author}",
                        icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
        await ctx.send(embed=success)
    except discord.Forbidden:
  ...
  async def changenickname(self, ctx: commands.Context, member: discord.Member, *, name: str = None):
    ...
    try:
        ...
        if name:
            ...
            success.set_author(name="Nickname Updated", icon_url=ctx.bot.user.display_avatar.url)
            success.set_footer(text=f"Requested by {ctx.author}",
                        icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
        else:
            success = discord.Embed(
            ...
            success.set_author(name="Nickname Cleared", icon_url=ctx.bot.user.display_avatar.url)
            success.set_footer(text=f"Requested by {ctx.author}",
                icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
        await ctx.send(embed=success)
    except discord.Forbidden:
      ...
      if interaction.user == ctx.author:
        if interaction.guild.me.guild_permissions.manage_channels:
          ...
          embed = discord.Embed(
            ...
            color=self.color)
          embed.set_author(name="Channel Nuked", icon_url=ctx.bot.user.display_avatar.url)
  ...
  async def _slowmode(self, ctx: commands.Context, seconds: int = 0):
    ...
    if seconds == 0:
      ...
      embed=discord.Embed(description="Successfully Set slowmode to **`%s`**" % (seconds),
                            color=self.color)
      embed.set_author(name="Slowmode Activated", icon_url=ctx.bot.user.display_avatar.url)
      embed.set_footer(text=f"Requested by {ctx.author}",
                        icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
  ...
  async def _unslowmode(self, ctx: commands.Context):
    ...
    embed.set_author(name="Unslowmode", icon_url=ctx.bot.user.display_avatar.url)
    embed.set_footer(text=f"Requested by {ctx.author}",
                        icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
    await ctx.send(embed=embed)
  ...
  async def roleicon(self, ctx: commands.Context, role: discord.Role, *, icon: Union[discord.Emoji, discord.PartialEmoji, str] = None):
    ...
    if role.position >= ctx.guild.me.top_role.position:
        ...
        error_embed.set_author(name="Error", icon_url=ctx.bot.user.display_avatar.url)
