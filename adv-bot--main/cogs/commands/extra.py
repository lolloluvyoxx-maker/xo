...
class RoleInfoView(View):
  ...
  @discord.ui.button(label='Show Permissions', style=discord.ButtonStyle.secondary)
...
class OverwritesView(View):
  ...
  async def show_overwrites(self, interaction: discord.Interaction, button: Button):
      ...
      embed = discord.Embed(title=f"Overwrites for {self.channel.name}", color=discord.Color(0xFFFFFF))
...
class Extra(commands.Cog):
  ...
  async def uptime(self, ctx):
      ...
      embed = discord.Embed(title=f"SALVATION Manager Uptime", color=self.color)
  ...
  async def serverinfo(self, ctx):
        embed = discord.Embed(color=0xFFFFFF).set_author(
...
**Color:** {member.color if member.color else '99aab5'}
  ...
  async def ping(self, ctx: commands.Context):
    ...
    initial_embed = discord.Embed(
        ...
        color=0xFFFFFF # Neutral dark color
    )
