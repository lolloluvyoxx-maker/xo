...
DB_PATH = "db/afk.db"
THEME_COLOR = 0xFFFFFF
FOOTER_TEXT = "Developed by SALVATION Development"
...
class afk(commands.Cog):
    ...
    async def afk(self, ctx: commands.Context, *, reason: str = "I am AFK"):
        if any(w in reason.lower() for w in ("discord.gg", "gg/")):
            return await ctx.send(embed=discord.Embed(description=" Advertising is not allowed in AFK reasons.", color=self.theme_color), ephemeral=True)
