...
class Customrole(commands.Cog):
    ...
    async def add_role(self, *, role_id: int, member: discord.Member):
        if member.guild.me.guild_permissions.manage_roles:
            ...
            await member.add_roles(role, reason="SALVATION Customrole | Role Added")
        else:
    ...
    async def remove_role(self, *, role_id: int, member: discord.Member):
        if member.guild.me.guild_permissions.manage_roles:
            role = discord.Object(id=role_id)
            await member.remove_roles(role, reason="SALVATION Customrole | Role Removed")
        else:
            raise discord.Forbidden("Bot does not have permission to manage roles.")
    ...
    async def reset(self, context: Context) -> None:
        if context.author == context.guild.owner or context.author.top_role.position > context.guild.me.top_role.position:
            ...
            color=0xFFFFFF
                ...
                embed = discord.Embed(description="No configuration found for this server.", color=0xFFFFFF)
                await context.reply(embed=embed)
        else:
            embed = discord.Embed(title="Access Denied",
                                  description="Your role should be above my top role.",
    ...
    async def on_message(self, message: discord.Message):
        ...
        if result:
            ...
            if reqrole is None:
                await message.channel.send("The required role is not set up in this server. Please set it up using `setup reqrole`.")
                return

            
            if reqrole not in message.author.roles:
                await message.channel.send(embed=discord.Embed(description=f"You need the {reqrole.mention} role to use this command.", color=0xFFFFFF))
                return
