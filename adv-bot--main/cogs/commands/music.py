...
class PlatformSelectView(View):
    ...
    async def perform_search(self, source):
        ...
        if not results:
            return await self.ctx.send(embed=discord.Embed(description="No results found.", color=0xFFFFFF))
...
class MusicControlView(View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        await interaction.response.send_message(
            embed=discord.Embed(description="Only members in the same voice channel as me can control the player.", color=0xFFFFFF),
            ephemeral=True
        )
        return False
    ...
    @discord.ui.button(label="Rewind 10s", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(label="Forward 10s", style=discord.ButtonStyle.secondary)
...
class Music(commands.Cog):
    ...
    async def inactivity_timer(self, guild):
        ...
        if len(guild.voice_channels[0].members) == 1:
            ...
            if player:
                ...
                try:
                    ended = discord.Embed(description="Bot has been disconnected due to inactivity (being idle in Voice Channel) for more than 2 minutes." , color=0xFFFFFF)
                    ended.set_author(name="Inactive Timeout", icon_url=self.client.user.avatar.url)
                    ended.set_footer(text="Thanks for choosing SALVATION!")
    ...
    async def display_player_embed(self, player, track, ctx, autoplay=False):
        if track.artwork:
            ...
            color=0xFFFFFF if "spotify" in track.source else 0xFFFFFF if "jiosaavn" in track.source else 0xFFFFFF if "youtube" in track.source else 0xFFFFFF
            )
    ...
    async def on_track_end(self, payload: wavelink.TrackEndEventPayload):
        ...
        if not player.queue:
            if player.queue.mode == wavelink.QueueMode.loop:
                ...
                ended = discord.Embed(description="All tracks have been played, leaving the voice channel." , color=0xFFFFFF)
    ...
    async def play_source(self, ctx, query):
        if not ctx.author.voice:
            await ctx.send(embed=discord.Embed(description="you need to be in a voice channel to use this command.", color=0xFFFFFF))
            return
        ...
        if vc.playing:
            if ctx.voice_client and ctx.voice_client.channel != ctx.author.voice.channel:
                await ctx.send(embed=discord.Embed(description=f"You must be connected to {ctx.voice_client.channel.mention} to play.", color=0xFFFFFF))
                return
        ...
        if not tracks:
            await ctx.send(embed=discord.Embed(description="No results found.", color=0xFFFFFF))
            return

        if isinstance(tracks, wavelink.Playlist):
            await vc.queue.put_wait(tracks.tracks)
            await ctx.send(embed=discord.Embed(description=f"Added playlist **{tracks.name}** with **{len(tracks.tracks)} songs** to the queue.", color=0xFFFFFF))
            if not vc.playing:
            ...
            track = tracks[0]
            await vc.queue.put_wait(track)
            await ctx.send(embed=discord.Embed(description=f"Added [{track.title}]({track.uri}) to the queue.", color=0xFFFFFF))
            if not vc.playing:
    ...
    async def handle_spotify_link(self, ctx, vc, link, type_):
        try:
            if type_ == "track":
                ...
                await ctx.send(embed=discord.Embed(description=f"Added **{c}** of **{playlist_length}** tracks from **playlist** **{playlist_info['name']}** to the queue.", color=0xFFFFFF))
                await lmao.delete()
                ...
                await ctx.send(embed=discord.Embed(description=f"Added all tracks from album **{album_info['name']}** to the queue.", color=0xFFFFFF))
                if not vc.playing:
                    next_track = await vc.queue.get_wait()
                    await vc.play(next_track)
                    await self.display_player_embed(vc, next_track, ctx)
    ...
    async def nowplaying(self, ctx: commands.Context):
        ...
        if not vc or not vc.playing:
            await ctx.send(embed=discord.Embed(description="No song is currently playing.", color=0xFFFFFF))
            return

        if not ctx.author.voice or ctx.author.voice.channel.id != vc.channel.id:
            await ctx.send(embed=discord.Embed(description="You need to be in the same voice channel as me to use this command.", color=0xFFFFFF))
            return
        ...
        embed = discord.Embed(
            title="Now Playing",
            color=0xFFFFFF if source_name == "Spotify" else 0xFFFFFF
        )
    ...
    async def autoplay(self, ctx: commands.Context):
        ...
        if vc:
            ...
            await ctx.send(embed=discord.Embed(description=f"Autoplay {'enabled' if vc.autoplay == wavelink.AutoPlayMode.enabled else 'disabled'} by {ctx.author.mention}.", color=0xFFFFFF))
    ...
    async def loop(self, ctx: commands.Context):
        ...
        if vc:
            ...
            await ctx.send(embed=discord.Embed(description=f"Loop {'enabled' if vc.queue.mode == wavelink.QueueMode.loop else 'disabled'} by {ctx.author.mention}.", color=0xFFFFFF))
        else:
            await ctx.send(embed=discord.Embed(description="I'm not connected to a voice channel.", color=0xFFFFFF))
    ...
    async def pause(self, ctx: commands.Context):
        ...
        if vc and vc.playing and not vc.paused:
            ...
            await ctx.send(embed=discord.Embed(description=f"Paused by {ctx.author.mention}.", color=0xFFFFFF))
        else:
            await ctx.send(embed=discord.Embed(description="  Nothing is playing or already paused.", color=0xFFFFFF))
    ...
    async def resume(self, ctx: commands.Context):
        ...
        if vc and vc.paused:
            ...
            await ctx.send(embed=discord.Embed(description=f"Resumed by {ctx.author.mention}.", color=0xFFFFFF))
        else:
            await ctx.send(embed=discord.Embed(description="Player is not paused.", color=0xFFFFFF))
    ...
    async def skip(self, ctx: commands.Context):
        ...
        if vc.autoplay == wavelink.AutoPlayMode.enabled:
            await vc.stop()
            return await ctx.send(embed=discord.Embed(description=f"Skipped by {ctx.author.mention}.", color=0xFFFFFF))


        if vc and vc.playing and not vc.queue.is_empty:
            await vc.stop()
            await ctx.send(embed=discord.Embed(description=f"Skipped by {ctx.author.mention}.", color=0xFFFFFF))
        else:
            await ctx.send(embed=discord.Embed(description="No song is playing or in the queue to skip.", color=0xFFFFFF))
    ...
    async def shuffle(self, ctx: commands.Context):
        ...
        if vc and vc.queue:
            ...
            await ctx.send(embed=discord.Embed(description=f"Queue shuffled by {ctx.author.mention}.", color=0xFFFFFF))
        else:
            await ctx.send(embed=discord.Embed(description="Queue is empty.", color=0xFFFFFF))
    ...
    async def stop(self, ctx: commands.Context):
        ...
        if vc and player:
            ...
            await ctx.send(embed=discord.Embed(description=f"Stopped and queue cleared by {ctx.author.mention}.", color=0xFFFFFF))
        else:
            await ctx.send(embed=discord.Embed(description="Nothing is playing to stop.", color=0xFFFFFF))
    ...
    async def volume(self, ctx: commands.Context, level: int):
        ...
        if vc:
            if 1 <= level <= 150:
                ...
                await ctx.send(embed=discord.Embed(description=f"Volume set to {level}% by {ctx.author.mention}.", color=0xFFFFFF))
            else:
                await ctx.send(embed=discord.Embed(description="Volume must be between 1 and 150.", color=0xFFFFFF))
        else:
            await ctx.send(embed=discord.Embed(description="Bot is not connected to a voice channel.", color=0xFFFFFF))
    ...
    async def queue(self, ctx: commands.Context):
        ...
        if not vc or not vc.queue or vc.queue.is_empty:
            await ctx.send(embed=discord.Embed(description="The queue is currently empty.", color=0xFFFFFF))
            return
    ...
    async def clearqueue(self, ctx: commands.Context):
        ...
        if not vc or not vc.queue or vc.queue.is_empty:
            await ctx.send(embed=discord.Embed(description="No Queue to clear.", color=0xFFFFFF))
            return
        ...
        if vc and vc.queue:
            vc.queue.clear()
            await ctx.send(embed=discord.Embed(description="Queue has been cleared.", color=0xFFFFFF))
        else:
            await ctx.send(embed=discord.Embed(description="No queue to clear.", color=0xFFFFFF))
    ...
    async def replay(self, ctx: commands.Context):
        ...
        if not vc or not vc.playing:
            await ctx.send(embed=discord.Embed(description="I'm not connected to any voice channel.", color=0xFFFFFF))
            return
        ...
        if vc and vc.playing:
            await vc.seek(0)
            await ctx.send(embed=discord.Embed(description="Replaying the current track.", color=0xFFFFFF))
        else:
            await ctx.send(embed=discord.Embed(description="No track is currently playing.", color=0xFFFFFF))
    ...
    async def join(self, ctx: commands.Context):
        if ctx.author.voice:
            ...
            await ctx.send(embed=discord.Embed(description="Joined the voice channel.", color=0xFFFFFF))
        else:
            await ctx.send(embed=discord.Embed(description="You need to join a voice channel first.", color=0xFFFFFF))
    ...
    async def disconnect(self, ctx: commands.Context):
        ...
        if vc:
            await vc.disconnect()
            await ctx.send(embed=discord.Embed(description="Disconnected from the voice channel.", color=0xFFFFFF))
        else:
            await ctx.send(embed=discord.Embed(description="Bot is not connected to any voice channel.", color=0xFFFFFF))
    ...
    async def seek(self, ctx: commands.Context, percentage: int):
        if not 1 <= percentage <= 100:
            await ctx.send(embed=discord.Embed(description="Please provide a percentage between 1 and 100.", color=0xFFFFFF))
            return
        ...
        await ctx.send(embed=discord.Embed(description=f"Seeked to {percentage}% of the current track.", color=0xFFFFFF))
