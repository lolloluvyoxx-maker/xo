...
class Giveaway(commands.Cog):
    ...
    async def gstart(self, ctx,
        ...
        if len(g_list) >= 5:
            embed = discord.Embed(title="Access Denied",
                                  description=f"You can only host upto 5 giveaways in this Guild.", color=0xFFFFFF)
            message = await ctx.send(embed=embed)
            await asyncio.sleep(5)
            await message.delete()
            return
        ...
        if converted / 60 >= 50400:
            embed = discord.Embed(title="Access Denied",
                                  description=f"Time cannot exceed 31 days!", color=0xFFFFFF)
            message = await ctx.send(embed=embed)
            await asyncio.sleep(5)
            await message.delete()
            return
        ...
        if converted == -1:
            embed = discord.Embed(title="Error",
                                  description=f"Invalid time format", color=0xFFFFFF)
            message = await ctx.send(embed=embed)
            await asyncio.sleep(5)
            await message.delete()
            return
    ...
    async def gend(self, ctx, message_id = None):
        if message_id:
            try:
                ...
                embed = discord.Embed(title="Access Denied",
                                      description="Invalid message ID provided.", color=0xFFFFFF)
                message = await ctx.send(embed=embed)
                await asyncio.sleep(5)
                await message.delete()
                return
        ...
        if message_id is not None:
            ...
            if re is None:
                embed = discord.Embed(title="Error",
                                      description=f" The giveaway was not found.", color=0xFFFFFF)
                message = await ctx.send(embed=embed)
                await asyncio.sleep(5)
                await message.delete()
                return
    ...
    async def greroll(self, ctx, message_id: typing.Optional[int] = None):
        ...
        if re is not None:
            embed = discord.Embed(title="Access Denied",
                                  description=f"The giveaway is currently running. Please use the `gend` command instead to end the giveaway.", color=0xFFFFFF)
            message = await ctx.send(embed=embed)
            await asyncio.sleep(5)
            await message.delete()
            return
    ...
    async def glist(self, ctx):
        ...
        if not giveaways:
            embed = discord.Embed(description=" No ongoing giveaways.", color=0xFFFFFF)
            await ctx.send(embed=embed)
            return

        embed = discord.Embed(title="Ongoing Giveaways", color=0xFFFFFF)
