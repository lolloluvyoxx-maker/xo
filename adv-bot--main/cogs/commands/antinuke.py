...
class Antinuke(commands.Cog):
  ...
  async def antinuke(self, ctx, option: str = None):
    ...
    if option is None:
      ...
      embed.set_footer(text="Your 24/7 Security Partner.", icon_url=self.bot.user.avatar.url)
      ...
      if is_activated:
        ...
        if not ctx.guild.me.guild_permissions.administrator:
          setup_embed.description += "\n| **Ops! It seems I Don't Have Administrator Perm To enable antinuke**."
          await setup_message.edit(embed=setup_embed)
          return

        await asyncio.sleep(1)
        setup_embed.description += "\nChecking SALVATION's role position for optimal configuration..."
        await setup_message.edit(embed=setup_embed)

        await asyncio.sleep(1)
        setup_embed.description += "\n| Crafting and configuring the SALVATION Supreme role..."
        await setup_message.edit(embed=setup_embed)
        
        try:
          role = await ctx.guild.create_role(
            name="SALVATION Supreme",
            color=0xFFFFFF,
          ...
          setup_embed.description += "\n| **Uh oh! I don't Have perms to enable antinuke**."
          await setup_message.edit(embed=setup_embed)
          return
        ...
        await asyncio.sleep(1)
        setup_embed.description += "\n| Ensuring precise placement of the SALVATION Supreme role..."
        await setup_message.edit(embed=setup_embed)
        try:
          await ctx.guild.edit_role_positions(positions={role: 1})
        except discord.Forbidden:
          setup_embed.description += "\n| Ops! I don't have sufficient perms to move role."
          await setup_message.edit(embed=setup_embed)
          return
        except discord.HTTPException as e:
          setup_embed.description += f"\n| Setup failed: HTTPException: {e}."
          await setup_message.edit(embed=setup_embed)
          return
        ...
        embed.set_author(name="SALVATION Antinuke", icon_url=self.bot.user.avatar.url)

        embed.set_footer(text="Successfully Enabled Antinuke for this server | Powered by SALVATION", icon_url=self.bot.user.avatar.url)
        embed.set_thumbnail(url=self.bot.user.avatar.url)
