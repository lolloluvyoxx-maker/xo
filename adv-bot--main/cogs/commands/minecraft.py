...
class Minecraft(commands.Cog):
    ...
    async def create_status_image(self, status_data, ip, port, server_type, server_icon_b64):
        try:
            ...
            power_text = "Powered by SALVATION"
    ...
    async def generate_response(self, guild, server_type, ip, port, user_id):
        ...
        embed = discord.Embed(title=f"{EMOJI_STATUS_ONLINE if is_online else EMOJI_STATUS_OFFLINE} Minecraft Server Status", color=discord.Color(0xFFFFFF) if is_online else discord.Color(0xFFFFFF))
