...
""")
    ...
    async def enable(self, ctx):
        if ctx.author.id != ctx.guild.owner_id:
            embed = discord.Embed(title="Error", description="Only the server owner can enable emergency mode.", color=0xFFFFFF)
        ...
        if roles_added:
            ...
            embed = discord.Embed(title="Error", description="No new roles with dangerous permissions were found.", color=0xFFFFFF)
    ...
    async def disable(self, ctx):
        if ctx.author.id != ctx.guild.owner_id:
            embed = discord.Embed(title="Error", description="Only the server owner can disable emergency mode.", color=0xFFFFFF)
            return await ctx.reply(embed=embed)
        ...
        embed = discord.Embed(title="Success", description="Emergency mode has been disabled, and all emergency roles have been cleared.", color=0xFFFFFF)
        await ctx.reply(embed=embed)
    ...
    async def authorise_add(self, ctx, member: discord.Member):
        if not await self.is_guild_owner(ctx):
            embed = discord.Embed(title="Error", description="Only the server owner can add authorised users for executing emergency situation.", color=0xFFFFFF)
            return await ctx.reply(embed=embed)
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            if count >= 5:
                embed = discord.Embed(title="Access Denied", description="Only up to 5 authorised users can be added.", color=0xFFFFFF)
                return await ctx.reply(embed=embed)

            async with db.execute("SELECT 1 FROM authorised_users WHERE guild_id = ? AND user_id = ?", (ctx.guild.id, member.id)) as cursor:
                if await cursor.fetchone():
                    embed = discord.Embed(title="Error", description="This user is already authorised.", color=0xFFFFFF)
                    return await ctx.reply(embed=embed)

            await db.execute("INSERT INTO authorised_users (guild_id, user_id) VALUES (?, ?)", (ctx.guild.id, member.id))
            await db.commit()

        embed = discord.Embed(title="Success", description=f"**{member.display_name}** has been authorised to use `emergency-situation` command.", color=0xFFFFFF)
        await ctx.reply(embed=embed)
    ...
    async def authorise_remove(self, ctx, member: discord.Member):
        if not await self.is_guild_owner(ctx):
            embed = discord.Embed(title="Access Denied", description="Only the server owner can remove authorised users for emergency situation.", color=0xFFFFFF)
            return await ctx.reply(embed=embed)

        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute("SELECT 1 FROM authorised_users WHERE guild_id = ? AND user_id = ?", (ctx.guild.id, member.id)) as cursor:
                if not await cursor.fetchone():
                    embed = discord.Embed(title="Error", description="This user is not authorised.", color=0xFFFFFF)
                    return await ctx.reply(embed=embed)

            await db.execute("DELETE FROM authorised_users WHERE guild_id = ? AND user_id = ?", (ctx.guild.id, member.id))
            await db.commit()

        embed = discord.Embed(title="Success", description=f"**{member.display_name}** has been removed from the authorised list and can no more use `emergency-situation` command.", color=0xFFFFFF)
        await ctx.reply(embed=embed)
    ...
    async def list_authorized(self, ctx):
        if not await self.is_guild_owner(ctx):
            embed = discord.Embed(title="Access Denied", description="Only the server owner can view the list of authorised users for emergency situation.", color=0xFFFFFF)
            return await ctx.reply(embed=embed)
    ...
    async def role_add(self, ctx, role: discord.Role):
        if not await self.is_guild_owner(ctx):
            embed = discord.Embed(title="Access Denied", description="Only the server owner can add role for emergency situation.", color=0xFFFFFF)
            return await ctx.reply(embed=embed)
        ...
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute("SELECT COUNT(*) FROM emergency_roles WHERE guild_id = ?", (ctx.guild.id,)) as cursor:
                count = (await cursor.fetchone())[0]
            if count >= 25:
                embed = discord.Embed(title="Error", description="Only up to 25 roles can be added.", color=0xFFFFFF)
                return await ctx.reply(embed=embed)

            async with db.execute("SELECT 1 FROM emergency_roles WHERE guild_id = ? AND role_id = ?", (ctx.guild.id, role.id)) as cursor:
                if await cursor.fetchone():
                    embed = discord.Embed(title="Error", description="This role is already in the emergency list.", color=0xFFFFFF)
                    return await ctx.reply(embed=embed)

            await db.execute("INSERT INTO emergency_roles (guild_id, role_id) VALUES (?, ?)", (ctx.guild.id, role.id))
            await db.commit()

        embed = discord.Embed(title="Success", description=f"**{role.name}** has been **added** to the emergency list.", color=0xFFFFFF)
        await ctx.reply(embed=embed)
    ...
    async def role_remove(self, ctx, role: discord.Role):
        if not await self.is_guild_owner(ctx):
            embed = discord.Embed(title="Access Denied", description="Only the server owner can remove roles from emergency list.", color=0xFFFFFF)
            return await ctx.reply(embed=embed)

        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute("SELECT 1 FROM emergency_roles WHERE guild_id = ? AND role_id = ?", (ctx.guild.id, role.id)) as cursor:
                if not await cursor.fetchone():
                    embed = discord.Embed(title="Error", description="This role is not in the emergency list.", color=0xFFFFFF)
                    return await ctx.reply(embed=embed)

            await db.execute("DELETE FROM emergency_roles WHERE guild_id = ? AND role_id = ?", (ctx.guild.id, role.id))
            await db.commit()

        embed = discord.Embed(title="Success", description=f"**{role.name}** has been removed from the emergency list.", color=0xFFFFFF)
        await ctx.reply(embed=embed)
    ...
    async def list_roles(self, ctx):
        if not await self.is_guild_owner_or_authorised(ctx):
            embed = discord.Embed(title="Access Denied", description="You are not authorised to view list of roles for emergency situation.", color=0xFFFFFF)
            return await ctx.reply(embed=embed)
    ...
    async def emergencysituation(self, ctx):
        ...
        processing_message = await ctx.send(embed=discord.Embed(title=" Processing Emergency Situation, wait for a while...", color=0xFFFFFF))
        ...
        if most_mem:
            ...
            try:
                ...
                await ctx.reply(embed=discord.Embed(
                    ...
                    description=f"**Roles Modified (Denied Dangerous Permissions)**:\n{success_message}\n\n** Role Moved**: {most_mem.mention} moved to a position below the bot's highest role.\n**Move back to its previous position soon after the server is not in risk.**\n\n** Errors**:\n{error_message}",
                    color=0xFFFFFF))
            except discord.Forbidden:
                await ctx.reply(embed=discord.Embed(
                    title="Emergency Situation",
                    description=f"**Roles Modified (Denied Dangerous Permissions)**:\n{success_message}\n\n**Role Couldn't Moved**: Failed to move the role {most_mem.mention} below the bot's highest role due to permissions error.\n**Move back to its previous position soon after the server is not in risk.**\n\n**Errors**:\n{error_message}",
                    color=0xFFFFFF))

            except Exception as e:
                await ctx.reply(embed=discord.Embed(
                    title="Emergency Situation",
                    description=f"**Roles Modified (Denied Dangerous Permissions)**:\n{success_message}\n\n**Role Couldn't Moved**: An unexpected error occurred while moving the role: {str(e)}.\n**Move back to its previous position soon after the server is not in risk.**\n\n** Errors**:\n{error_message}",
                    color=0xFFFFFF))
        else:
            await ctx.reply(embed=discord.Embed(
                title="Emergency Situation",
