...
class HelpCommand(commands.HelpCommand):
  ...
  async def send_bot_help(self, mapping):
    ...
    embed.set_footer(
      text=f"Requested By {self.context.author} | [Support](https://discord.gg/your-server-here)",
    )
  ...
  async def send_cog_help(self, cog):
    ...
    paginator = Paginator(source=FieldPagePaginator(
      entries=entries,
      title=f"SALVATION's {cog.qualified_name.title()} ({len(cog.get_commands())})",
