...
class ShowRules(discord.ui.View):
    ...
    async def show_rules(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = discord.Embed(title="Enabled Automod Rules", description=enabled_rules, color=0xFFFFFF)
...
class Automod(commands.Cog):
    ...
    async def enable(self, ctx):
        ...
        if ctx.author != ctx.guild.owner and ctx.author.top_role.position < ctx.guild.me.top_role.position:
            embed = discord.Embed(title="Access Denied", description="Your top role must be at the **same** position or **higher** than my top role.", color=0xFFFFFF)
        ...
        embed = discord.Embed(title=f"{ctx.guild.name}'s Automod Setup", color=0xFFFFFF)
        embed.set_thumbnail(url=self.bot.user.avatar.url)
    ...
    async def enable_automod(self, ctx, guild_id, selected_events, interaction):
        ...
        embed = discord.Embed(title="Automod Enabled Successfully", color=0xFFFFFF)
        ...
        async def enable_logging_callback(interaction):
            ...
            try:
                for channel in interaction.guild.channels:
                    if channel.name == "salvation-automod":
                        await interaction.response.send_message(f"A logging channel with the name \"salvation-automod\" already exists.", ephemeral=True)
                        return
                log_channel = await interaction.guild.create_text_channel("salvation-automod", overwrites=overwrites)
    ...
    async def punishment(self, ctx):
        ...
        embed = discord.Embed(title=f"Current Automod Punishments for {ctx.guild.name}", color=0xFFFFFF)
        punishment_map = {}
        ...
        async def select_callback(interaction):
            ...
            for punishment in ["Mute", "Kick", "Ban"]:
                ...
                async def punishment_callback(button_interaction, selected_events=selected_events, punishment=punishment):
                    ...
                    updated_embed = discord.Embed(title=f"Updated Automod Punishments for {ctx.guild.name}", color=0xFFFFFF)
    ...
    async def ignore_show(self, ctx):
        ...
        embed = discord.Embed(title="Ignored Channels & Roles for Automod", color=0xFFFFFF)
    ...
    async def unignore_channel(self, ctx, channel: discord.TextChannel):
        ...
        if result.rowcount > 0:
            embed = discord.Embed(title="Success", description=f"{channel.mention} has been removed from the automod ignore list.", color=0xFFFFFF)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
            await ctx.send(embed=embed)
        else:
            embed = discord.Embed(title="Error", description=f"{channel.mention} is not in the automod ignore list.", color=0xFFFFFF)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
            await ctx.send(embed=embed)
    ...
    async def unignore_role(self, ctx, role: discord.Role):
        ...
        if result.rowcount > 0:
            embed = discord.Embed(title="Success", description=f"{role.mention} has been removed from the automod ignore list.", color=0xFFFFFF)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
            await ctx.send(embed=embed)
        else:
            embed = discord.Embed(title="Error", description=f"{role.mention} is not in the automod ignore list.", color=0xFFFFFF)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
            await ctx.send(embed=embed)
    ...
    async def config(self, ctx):
        ...
        current_punishments = await self.get_current_punishments(guild_id)
        embed = discord.Embed(title=f"Enabled Automod Events & their punishment type for {ctx.guild.name}", color=0xFFFFFF)
