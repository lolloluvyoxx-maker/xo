...
MAPQUEST_API_KEY = os.getenv("MAPQUEST_API_KEY", "E2SaL3qiTpXQ43nxZFBp0wzEnBI6pqbG")
...
class MapView(ui.View):
    ...
    def get_coordinates(self, location):
        ...
        try:
            headers = {'User-Agent': 'SALVATION Bot (https://sorrylinknahithi)'}
    ...
    async def update_embed(self, interaction: discord.Interaction):
        if self.latitude is None or self.longitude is None:
            ...
            return
        embed = discord.Embed(title=f"Map of {self.location}", color=0xFFFFFF)
        ...
        embed.set_footer(text="Made by SALVATION")
        try:
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user != self.ctx.author:
            await interaction.response.send_message(
                embed=discord.Embed(description="Sorry only the requested author can control this", color=0xFFFFFF),
                ephemeral=True
            )
            return False
        return True

    @discord.ui.button(label="West", style=ButtonStyle.secondary)
    ...
    @discord.ui.button(label="North", style=ButtonStyle.secondary)
    ...
    @discord.ui.button(label="South", style=ButtonStyle.secondary)
    ...
    @discord.ui.button(label="East", style=ButtonStyle.secondary)
...
class Map(commands.Cog):
    ...
    async def map(self, ctx, *, location: str):
        ...
        if view.coordinates == (None, None):
            ...
            return
        embed = discord.Embed(title=f"Map of {location}", color=0xFFFFFF)
