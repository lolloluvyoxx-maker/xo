...
class Booster (Cog ):
    def __init__ (self ,bot :SALVATION ):
        self .bot =bot
        self .color =0xFFFFFF
    ...
    async def _boost_thumbnail (self ,ctx ,thumbnail_url :str ):
        ...
        embed =discord .Embed (
        color =self .color ,
        description ="Successfully updated the boost thumbnail URL.",
    ...
    async def _boost_image (self ,ctx ,*,image_url :str ):
        ...
        embed =discord .Embed (
        color =self .color ,
        description ="Successfully updated the boost image URL.",
        timestamp =ctx .message .created_at
        )
        embed .set_author (name =ctx .author .name ,icon_url =ctx .author .display_avatar .url )
        await ctx .send (embed =embed )
    ...
    async def _boost_autodel (self ,ctx ,seconds :int ):
        ...
        if seconds <0 :
            await ctx .send (embed =discord .Embed (
            description ="Auto-delete timer must be 0 or greater.",
            color =self .color
            ))
        ...
        description =f"Successfully set auto-delete timer to {seconds} seconds."
        if seconds ==0 :
            description ="Auto-delete has been disabled."
    ...
    async def _boost_message (self ,ctx ):
        ...
        embed =discord .Embed (
        color =self .color ,
        description ="Successfully updated the boost message.",
        timestamp =ctx .message .created_at
        )
        embed .set_author (name =ctx .author .name ,icon_url =ctx .author .display_avatar .url )
        await ctx .send (embed =embed )
    ...
    async def _boost_embed (self ,ctx ):
        ...
        embed =discord .Embed (
        color =self .color ,
        description =f"Embed formatting has been **{status}**.",
        timestamp =ctx .message .created_at
        )
        embed .set_author (name =ctx .author .name ,icon_url =ctx .author .display_avatar .url )
        await ctx .send (embed =embed )
    ...
    async def _boost_ping (self ,ctx ):
        ...
        embed =discord .Embed (
        color =self .color ,
        description =f"Booster pinging has been **{status}**.",
        timestamp =ctx .message .created_at
        )
        embed .set_author (name =ctx .author .name ,icon_url =ctx .author .display_avatar .url )
        await ctx .send (embed =embed )
    ...
    async def _boost_channel_add (self ,ctx ,channel :discord .TextChannel ):
        ...
        if len (channels )>=3 :
            embed =discord .Embed (
            color =self .color ,
            description ="Maximum boost channel limit reached (3 channels).",
            timestamp =ctx .message .created_at
            )
            embed .set_author (name =ctx .author .name ,icon_url =ctx .author .display_avatar .url )
            await ctx .send (embed =embed )
        ...
        if str (channel .id )in channels :
            embed =discord .Embed (
            color =self .color ,
            description ="This channel is already in the boost channels list.",
            timestamp =ctx .message .created_at
            )
            embed .set_author (name =ctx .author .name ,icon_url =ctx .author .display_avatar .url )
            await ctx .send (embed =embed )
        ...
        embed =discord .Embed (
        color =self .color ,
        description =f"Successfully added {channel.mention} to boost channels list.",
        timestamp =ctx .message .created_at
        )
        embed .set_author (name =ctx .author .name ,icon_url =ctx .author .display_avatar .url )
        await ctx .send (embed =embed )
    ...
    async def _boost_channel_remove (self ,ctx ,channel :discord .TextChannel ):
        ...
        if not channels :
            embed =discord .Embed (
            color =self .color ,
            description ="No boost channels are currently set up.",
            timestamp =ctx .message .created_at
            )
            embed .set_author (name =ctx .author .name ,icon_url =ctx .author .display_avatar .url )
            await ctx .send (embed =embed )
        ...
        if str (channel .id )not in channels :
            embed =discord .Embed (
            color =self .color ,
            description ="This channel is not in the boost channels list.",
            timestamp =ctx .message .created_at
            )
            embed .set_author (name =ctx .author .name ,icon_url =ctx .author .display_avatar .url )
            await ctx .send (embed =embed )
        ...
        embed =discord .Embed (
        color =self .color ,
        description =f"Successfully removed {channel.mention} from boost channels list.",
        timestamp =ctx .message .created_at
        )
        embed .set_author (name =ctx .author .name ,icon_url =ctx .author .display_avatar .url )
        await ctx .send (embed =embed )
    ...
    async def _boost_test (self ,ctx ):
        ...
        if not channels :
            embed =discord .Embed (
            color =self .color ,
            description ="Please set up a boost channel first using `boost channel add #channel`.",
            timestamp =ctx .message .created_at
            )
            embed .set_author (name =ctx .author .name ,icon_url =ctx .author .display_avatar .url )
            await ctx .send (embed =embed )
        ...
        try :
            ...
            await ctx .send (embed =discord .Embed (
            description ="I don't have permission to send messages in the boost channel.",
            color =self .color
            ))
    ...
    async def _boost_config (self ,ctx ):
        ...
        embed =discord .Embed (
        color =self .color ,
        title =f"Boost Configuration for {ctx.guild.name}"
        )
        ...
        embed .add_field (
        name ="Embed",
        value ="Enabled"if data ["boost"]["embed"]else "Disabled",
        inline =True
        )

        embed .add_field (
        name ="Ping",
        value ="Enabled"if data ["boost"]["ping"]else "Disabled",
        inline =True
        )
    ...
    async def _boost_reset (self ,ctx ):
        ...
        if not data ["boost"]["channel"]:
            embed =discord .Embed (
            description ="No boost configuration found to reset.",
            color =self .color
            )
            await ctx .send (embed =embed )
            return
        ...
        embed =discord .Embed (
        description ="Successfully reset all boost configuration.",
        color =self .color
        )
        await ctx .send (embed =embed )
    ...
    async def _boostrole_add (self ,ctx ,role :discord .Role ):
        ...
        if len (roles )>=10 :
            embed =discord .Embed (
            description ="Maximum boost role limit reached (10 roles).",
            color =self .color
            )
            await ctx .send (embed =embed )
            return
        ...
        if str (role .id )in roles :
            embed =discord .Embed (
            description =f"{role.mention} is already a boost role.",
            color =self .color
            )
            await ctx .send (embed =embed )
            return
        ...
        embed =discord .Embed (
        description =f"{role.mention} has been added as a boost role.",
        color =self .color
        )
        await ctx .send (embed =embed )
    ...
    async def _boostrole_remove (self ,ctx ,role :discord .Role ):
        ...
        if not roles :
            embed =discord .Embed (
            description ="No boost roles are currently configured.",
            color =self .color
            )
            await ctx .send (embed =embed )
            return
        ...
        if str (role .id )not in roles :
            embed =discord .Embed (
            description =f"{role.mention} is not a boost role.",
            color =self .color
            )
            await ctx .send (embed =embed )
            return
        ...
        embed =discord .Embed (
        description =f"{role.mention} has been removed from boost roles.",
        color =self .color
        )
        await ctx .send (embed =embed )
    ...
    async def _boostrole_reset (self ,ctx ):
        ...
        embed =discord .Embed (
        description ="Successfully cleared all boost roles.",
        color =self .color
        )
        await ctx .send (embed =embed )
