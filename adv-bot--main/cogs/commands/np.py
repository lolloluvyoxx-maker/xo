...
class TimeSelect(Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        log_channel = interaction.client.get_channel(int(os.environ["LOG_CHANNEL_ID"]) if os.environ.get("LOG_CHANNEL_ID") else None)
        if log_channel:
            embed = discord.Embed(
...
class NoPrefix(commands.Cog):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            if expired_users:
                ...
                for user_id in expired_users:
                    ...
                    if user:
                        log_channel = self.client.get_channel(int(os.environ["LOG_CHANNEL_ID"]) if os.environ.get("LOG_CHANNEL_ID") else None)
                        if log_channel:
                        ...
                        embed = discord.Embed(
                            description=f"Your No Prefix status has **Expired**. You will now require the prefix to use commands.",
                            color=0xFFFFFF
                        )
                        embed.set_author(name="No Prefix Expired", icon_url=user.avatar.url if user.avatar else user.default_avatar.url)
                        
                        embed.set_footer(text="SALVATION  - No Prefix, Join support to regain access.")
                        support = Button(label='Support',
                    style=discord.ButtonStyle.link,
                    url=f'https://discord.gg/your-server-here')
                        view = View()
    ...
    async def np_add(self, ctx, user: discord.User):
        ...
        embed = discord.Embed(title="Select No Prefix Duration", description="**Choose the duration for how long no-prefix should be enabled for this user:**", color=0xFFFFFF)
    ...
    async def np_remove(self, ctx, user: discord.User):
        ...
        log_channel = ctx.bot.get_channel(int(os.environ["LOG_CHANNEL_ID"]) if os.environ.get("LOG_CHANNEL_ID") else None)
        if log_channel:
            embed_log = discord.Embed(
    ...
    async def add_np(self, user, duration):
        ...
        embed = discord.Embed(
                            ...
                            description=f"You've been credited 2 months of global No Prefix for boosting our Partnered Servers. You can now use my commands without prefix. If you wish to remove it, please reach out [Support Server](https://discord.gg/your-server-here).",
                            color=0xFFFFFF
                        )
        try:
    ...
    async def remove_np(self, user):
        ...
        embed= discord.Embed(title="Global No Prefix Expired",
                        description=f"Hey {user.mention}, your global no prefix has expired!\n\n__**Reason:**__ Unboosting our partnered Server.\nIf you think this is a mistake then please reach out [Support Server](https://discord.gg/your-server-here).",
                        color=0xFFFFFF)
    ...
    async def np_reset(self, ctx):
        ...
        async def yes_callback(interaction):
            ...
            success_embed = discord.Embed(
                title="No-Prefix Reset Complete",
