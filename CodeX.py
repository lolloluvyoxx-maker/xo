...
# --- Configuration ---
# IMPORTANT: Set these as environment variables to your own channel IDs to enable
# these optional features. They safely no-op (do nothing) if left unset.
SERVER_COUNT_CHANNEL_ID = int(os.environ["SERVER_COUNT_CHANNEL_ID"]) if os.environ.get("SERVER_COUNT_CHANNEL_ID") else None
USER_COUNT_CHANNEL_ID = int(os.environ["USER_COUNT_CHANNEL_ID"]) if os.environ.get("USER_COUNT_CHANNEL_ID") else None
...
async def on_guild_join(guild: discord.Guild):
    ...
    if log_channel:
        await log_channel.send(f"SALVATION has been added to the server: **{guild.name}** (ID: `{guild.id}`)")
...
async def list_hooks(ctx: Context):
    ...
    try:
        ...
        embed = discord.Embed(title=f"Webhooks in #{ctx.channel.name}", color=0xFFFFFF)
...
@app.route('/')
def home():
    return f"SALVATION is online"

def run():
    port = int(os.environ.get("PORT", 19346))
