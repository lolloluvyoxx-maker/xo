...
class Fun(commands.Cog):
    ...
    async def red_footer(self, embed, ctx):
        embed.set_footer(text="SALVATION Development™ Pro Mode", icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
    ...
    async def toxic(self, ctx, user: discord.Member = None):
        ...
        embed = discord.Embed(title=" Toxic Meter", description=f"**{user.mention} is {toxic_level}% toxic!**")
        await ctx.send(embed=await self.red_footer(embed, ctx))
