...
class Jail(commands.Cog):
    ...
    async def unjail_member(self, guild, member):
        ...
        if log_channel_id:
            ...
            if log_channel:
                embed = discord.Embed(title=" Member Unjailed", color=discord.Color(0xFFFFFF))
    ...
    async def jail(self, ctx, member: discord.Member, duration: str = None, *, reason="No reason provided"):
        ...
        if not jail_role_id or not jail_channel_id:
            return await ctx.send(" Jail system not fully configured.")

        jail_role = ctx.guild.get_role(int(jail_role_id))
        if not jail_role:
            return await ctx.send(" Jail role does not exist.")
        ...
        if log_channel_id:
            ...
            if log_channel:
                embed = discord.Embed(title=" Member Jailed", color=discord.Color(0xFFFFFF))
    ...
    async def jailhistory(self, ctx, member: discord.Member):
        ...
        if row:
            ...
            jailed_time = datetime.fromisoformat(jailed_at)
            embed = discord.Embed(title=" Jail Record", color=discord.Color(0xFFFFFF))
            embed.add_field(name="User", value=member.mention)
