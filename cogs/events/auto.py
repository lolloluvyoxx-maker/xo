    ...
    async def send_msg_to_adder(self, guild: discord.Guild):
        async for entry in guild.audit_logs(limit=3):
            if entry.action == discord.AuditLogAction.bot_add:
                embed = discord.Embed(
                   description=f"**Thanks for adding me.**\n\nMy default prefix is `>`\n> Use the `>help` command to see a list of commands\nFor detailed guides, FAQ and information, visit our **[Support Server](https://discord.gg/your-server-here)**",
                    color=0xFFFFFF
               )
                ...
                support_button = Button(label='Support', style=discord.ButtonStyle.link, url='https://discord.gg/your-server-here')
